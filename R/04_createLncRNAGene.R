...
#'
#' This function creates a \code{LncRNAGene} object, which represents
#' a gene that encodes for long non-coding RNAs. The LncRNAGene object
#' includes base information inherited from the \code{Gene} class 
#' with the specific information regarding its lncRNA product.
#' 
...
#' Clinical relevance of the gene or its product (e.g., association with a 
#' disease or therapeutic targets). If not specified, its default will be "-".
#' 
#' @return A \code{LncRNAGene} object.
#' 
...
#'   creates the GRanges object, so this check is not implemented in the 
#'   package.
#'   \item The \code{gene_product} slot is a list that represents a gene 
#'   product and contains two elements: the lncrna_id and the lncrna_sequence.
...
createLncRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                             description = NA_character_, 
                             tissue_specificity = list(), 
                             gene_structure, gene_product, 
                             clinical_significance = NA_character_) {
  new("LncRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
