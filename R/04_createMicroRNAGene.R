#' Create a Micro RNA Gene object
#'
#' This function creates a \code{MicroRNAGene} object, which represents
#' a gene that encodes for microRNAs. The MicroRNAGene object includes 
#' base information inherited from the \code{Gene} class 
#' with the specific information regarding its miRNA product.
#' 
#' @param id character. Ensembl transcript ID of the gene. In this 
#' implementation, each transcript isoform is considered as an individual 
#' gene. Hence, the Ensembl transcript ID ("ENST") is used instead of the 
#' Ensembl gene ID ("ENSG"). This approach simplifies the management of Gene 
#' objects, avoiding the complexity of handling multiple gene products of 
#' different types within a single Gene object.
...
#' (character) where the gene is specifically expressed. If not specified, 
#' its default will be "-".
...
#' 
#' @return A \code{MicroRNAGene} object.
#' 
...
#'   equal to 'end' when the user creates the GRanges
#'   object, so this check is not implemented in the package.
#'   \item The \code{gene_product} slot is a list that represents a gene 
#'   product and contains two elements: the microrna_id and 
#'   the microrna_sequence.
...
#'                   strand = "+")
#'                          
#' gene_product <- list(microrna_id = "miRNAID", 
#'                      microrna_sequence = "UAUGCACUGAUCCUAGCAUUG")
#' 
#' microrna_gene <- createMicroRNAGene(id = "ENST000001",
#'                  hugo_symbol = "SYMBOL1",
#'                  name = "miRNA gene name",
...
createMicroRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                               description = NA_character_, 
                               tissue_specificity = list(), 
                               gene_structure, gene_product, 
                               clinical_significance = NA_character_) {
  new("MicroRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
