#' Create a Piwi-interacting RNA Gene object
#'
#' This function creates a \code{PiRNAGene} object, which represents a 
#' gene that encodes for Piwi interacting RNAs. The PiRNAGene object 
#' includes base information inherited from the \code{Gene} class 
#' with the specific information regarding its piRNA product.
#' 
...
#' 
#' @return A \code{PiRNAGene} object.
#' 
...
#'   product and contains two elements: the pirna_id and the pirna_sequence.
...
#'                   strand = "+")
#'                          
#' gene_product <- list(pirna_id = "piRNAID", 
#'                      pirna_sequence = "CUAUCGGCUUACGAUACCUUGGAUACGGA")
#' 
#' pirna_gene <- createPiRNAGene(id = "ENST000001",
#'               hugo_symbol = "SYMBOL1",
...
createPiRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                            description = NA_character_, 
                            tissue_specificity = list(), 
                            gene_structure, gene_product, 
                            clinical_significance = NA_character_) {
  new("PiRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
