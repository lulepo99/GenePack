#' Create a Ribosomal RNA Gene object
#'
#' This function creates a \code{RRNAGene} object, which represents a 
#' gene that encodes for ribosomal RNAs. The RRNAGene object includes 
#' base information inherited from the \code{Gene} class with the 
#' specific information regarding its rRNA product.
#' 
...
#' Gene objects, avoiding the 
#' complexity of handling multiple gene products of different types 
#' within a single Gene object.
...
#'   two elements: the rrna_id and the rrna_sequence.
...
#' separately by the user before constructing the Gene object. This approach 
#' maximizes flexibility, allowing 
...
#'                   strand = "+")
#'                          
#' gene_product <- list(rrna_id = "rRNAID", 
#'                  rrna_sequence = paste0(
#'                          "GGUCUAUGCCUUCUAAGGGAUUGGCU",
#'                          "ACCUAAGGUACUCGGGAUCUUGAU"))
#' 
#' rrna_gene <- createRRNAGene(id = "ENST000001",
#'              hugo_symbol = "SYMBOL1",
#'              name = "rRNA gene name",
...
createRRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                           description = NA_character_, 
                           tissue_specificity = list(), 
                           gene_structure, gene_product, 
                           clinical_significance = NA_character_) {
  new("RRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
