#' Create a Short-interfering RNA Gene object
#'
#' This function creates a \code{SiRNAGene} object, which represents a 
#' gene that encodes for short interfering RNAs. The SiRNAGene object 
#' includes base information inherited from the \code{Gene} class
#' with the specific information regarding its siRNA product.
#' 
...
#' Gene objects, avoiding the complexity of handling multiple gene 
#' products of different types within a single Gene object.
...
#' 
#' @return A \code{SiRNAGene} object.
#' 
...
#'   two elements: the sirna_id and the sirna_sequence.
...
#'                   strand = "+")
#'                          
#' gene_product <- list(sirna_id = "siRNAID", 
#'                       sirna_sequence = "AUCGGACUUAUGCGUUUACGG")
#' 
#' sirna_gene <- createSiRNAGene(id = "ENST000001",
#'               hugo_symbol = "SYMBOL1",
...
createSiRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                            description = NA_character_, 
                            tissue_specificity = list(), 
                            gene_structure, gene_product, 
                            clinical_significance = NA_character_) {
  new("SiRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
