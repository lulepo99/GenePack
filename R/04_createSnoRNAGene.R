#' Create a Small Nucleolar RNA Gene object
#'
#' This function creates a \code{SnoRNAGene} object, which represents 
#' a gene that encodes for small nucleolar RNAs. The SnoRNAGene object 
#' includes base information inherited from the \code{Gene} class 
#' with the specific information regarding its snoRNA product.
#' 
...
#' 
#' @return A \code{SnoRNAGene} object.
#' 
...
#'   object in \code{GRanges} directly verify that 'start' is less than 
#'   or equal to 'end' when the user creates the GRanges
#'   object, so this check is not implemented in the package.
#'   \item The \code{gene_product} slot is a list that represents a 
#'   gene product and contains two elements: the snorna_id and 
#'   the snorna_sequence.
...
#'                   strand = "+")
#'                          
#' gene_product <- list(snorna_id = "snoRNAID", 
#'                    snorna_sequence = paste0(
#'                        "GUAUCGAUCUAUGCAUCCUACGUACGGAU",
#'                        "GCUAACGUUCCGGAUUGCUAAUGGCUAAC"))
#' 
#' snorna_gene <- createSnoRNAGene(id = "ENST000001",
#'                hugo_symbol = "SYMBOL1",
...
createSnoRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                             description = NA_character_, 
                             tissue_specificity = list(), 
                             gene_structure, gene_product, 
                             clinical_significance = NA_character_) {
  new("SnoRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
