#' Create a Transfer RNA Gene object
#'
#' This function creates a \code{TRNAGene} object, which represents a gene 
#' that encodes for transfer RNAs. The TRNAGene object includes base 
#' information inherited from the \code{Gene} class 
#' with the specific information regarding its tRNA product.
#' 
...
#'   two elements: the trna_id and the trna_sequence.
...
#'                   strand = "+")
#'                          
#' gene_product <- list(trna_id = "tRNAID", 
#'                      trna_sequence = paste0(
#'                          "GCCUAUACGUUACGAUCUCGAAUAGCCUUACGUUAACCG",
#'                          "UAUGCUAUGGUGACCGUAUGCUAUGGUACGUACGAA"))
#' 
#' trna_gene <- createTRNAGene(id = "ENST000001",
#'              hugo_symbol = "SYMBOL1",
#'              name = "tRNA gene name",
...
createTRNAGene <- function(id, hugo_symbol, name = NA_character_, 
                           description = NA_character_, 
                           tissue_specificity = list(), 
                           gene_structure, gene_product, 
                           clinical_significance = NA_character_) {
  new("TRNAGene", id = id, hugo_symbol = hugo_symbol, name = name, 
